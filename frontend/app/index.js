...
export default function HomeScreen() {
  ...
  const { saved, baseline } = useMemo(() => ({ saved: 38.5, baseline: 150 }), []);
  ...
  const upcoming = useMemo(
    () => [
      { id: "banana", name: "Banana", days: 2, icon: "food-apple", color: "#F59E0B" },
      { id: "chicken", name: "Chicken", days: 3, icon: "food-drumstick", color: "#10B981" },
    ],
    []
  );
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: "#F4F6F5" }} contentContainerStyle={styles.wrap}>
      {/* Top bar: logo + avatar */}
      <View style={styles.topBar}>
        ...
        <TouchableOpacity onPress={() => router.push("profile")} style={styles.avatar}>
          <MaterialCommunityIcons name="account-circle" size={28} color="#0E0E0E" />
        </TouchableOpacity>
      </View>
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.9}
        onPress={() => router.push({ pathname: "camera", params: { type: "receipt" } })}
      >
      ...
      {/* Upcoming Expiries */}
      <TouchableOpacity style={styles.sectionHeader} onPress={() => router.push("notifications")} activeOpacity={0.7}>
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          <MaterialCommunityIcons name="bell-outline" size={18} color="#0E0E0E" />
      ...
      <View style={{ gap: 10 }}>
        {visibleUpcoming.map((it) => (
          <View key={it.id} style={styles.expiryRow}>
            <MaterialCommunityIcons name={it.icon} size={20} color="#0E0E0E" style={{ marginRight: 8 }} />
            ...
            <View style={{ flexDirection: "row", gap: 10, marginLeft: 8 }}>
              {/* X — hide only on Home */}
              <TouchableOpacity style={styles.iconPill} onPress={() => hideOnce(it.id)}>
                <MaterialCommunityIcons name="close" size={16} color="#0E0E0E" />
              </TouchableOpacity>

              {/* ✓ — also hide only on Home (no global state) */}
              <TouchableOpacity style={styles.iconPill} onPress={() => hideOnce(it.id)}>
                <MaterialCommunityIcons name="check" size={16} color="#0E0E0E" />
              </TouchableOpacity>
            </View>
          </View>
        ))}
      ...
      <View style={styles.sectionHeaderStatic}>
        ...
        <MaterialCommunityIcons name="silverware-fork-knife" size={16} color="#0E0E0E" />
      </View>
      <View style={styles.grid}>
        <Tile
          title="Pantry"
          icon="archive-outline"
          onPress={() => router.push({ pathname: "storage", params: { type: "pantry" } })}
        />
        <Tile
          title="Fridge"
          icon="fridge-outline"
          onPress={() => router.push({ pathname: "storage", params: { type: "fridge" } })}
        />
      </View>
      ...
      <View style={styles.grid}>
        <Tile title="Recipes" icon="book-outline" onPress={() => router.push("recipes")} />
        <Tile
          title="AI Advice"
          icon="robot-outline"
          onPress={() => router.push({ pathname: "camera", params: { type: "food" } })}
        />
      </View>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { marginLeft: 6, fontWeight: "800", fontSize: 14, color: "#0E0E0E" },
