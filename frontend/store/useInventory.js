...
const sumAllQty = (s) => sumQty(s.pantry) + sumQty(s.fridge) + sumQty(s.freezer);

// helpers
const mapDec = (list, id, count) =>
  list
    .map((x) => (x.id === id ? { ...x, qty: clampNonNeg((x.qty || 0) - count) } : x))
    .filter((x) => (x.qty || 0) > 0);
...
const atRisk = (items, threshold) => items.filter((x) => (x.daysLeft ?? 0) <= threshold);
...
const useInventory = create((set, get) => ({
  ...
  getMonthlyStats: () => {
    const s = get();
    const m = monthKey();
    const consumed = s.history.filter((e) => e.month === m && e.action === "consumed");
    const discarded = s.history.filter((e) => e.month === m && e.action === "discarded");
